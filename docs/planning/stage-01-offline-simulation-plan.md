...
- 在 bat 入口稳定后，可以额外提供一个 `Windows GUI exe 启动器`，但它只能作为现有 bat 入口的包装层
...
- 当前如果提供 `.exe`，其职责也应只是收集参数、启动 bat、显示进度和打开结果
...
- `ManualSelection`
  - 面向“部分手动指定、部分随机补齐”的实验对局
  - 输入为 `HeroCatalog + 蓝红双方手动槽位`
  - 未指定的槽位由程序在剩余英雄池中无放回随机补齐
...
- `ManualSelection`
  - 用户或 GUI 工具可以只指定蓝红双方的一部分英雄槽位
  - 程序必须保留指定槽位不变，并从剩余英雄池中 `无放回` 随机补齐空槽位
  - 手动指定的英雄之间、以及手动指定与随机补齐之间，都必须保持单局全局唯一
  - 如果手动指定里已经出现重复 heroId，离线模拟应直接报错终止
