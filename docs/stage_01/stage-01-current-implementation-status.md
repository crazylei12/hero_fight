...
- 火法已经形成 `普攻投射物 + 小技能范围 + 大招范围` 的项目专用 prefab-first VFX 示例链路

当前仍未完成的核心部分：
- 技能系统目前仍是第一版，不是完整版本
- 状态系统目前只接入了第一版属性修正
- 正式主通路已经打通，但 UI 仍是第一版 IMGUI 方案
- HeroSelect、Result 已可用，但视觉和交互仍未打磨
- 正式战斗日志面板仍未完成
- 完整的 prefab-first 战斗 VFX 样例目前主要集中在火法，其它英雄仍以通用或占位表现为主
...
- 技能动画仍未系统化接入
- 但火法普攻投射物、小技能范围与大招范围已经接入项目专用 VFX prefab
- 技能系统还没有完整职业策略加成
...
- 用平面 sprite 或项目专用 prefab 显示远程普攻投射物与技能范围
- 顶部显示正式战斗 HUD
- 正式 HUD 显示时间、比分与结束结果
...
- `game/Assets/Scenes/BattleBasicAttackOnly.unity`

说明：
- `MainMenu.unity` 现在是正式主入口
- `HeroSelect.unity` 现在可直接组装双方 5 人阵容
- `Battle.unity` 现在是正式主战斗场景，战斗结束后会自动进入结果页
- `Result.unity` 用于展示胜负、比分与英雄统计
- `BattleBasicAttackOnly.unity` 仅保留给开发验证，不属于玩家主流程
...
- `FIREMAGE.asset` 引用 `FireMageBasicAttackProjectile.prefab`
- `Ember Burst.asset` 引用 `FireMageEmberBurst.prefab`
- `Meteor Fall.asset` 引用 `FireMageMeteorField.prefab`
- `Stage01SampleContentBuilder.cs` 已与这三条引用同步
...
3. 打开 `game/Assets/Scenes/MainMenu.unity`
4. 点击 Play
5. 进入 `MainMenu -> HeroSelect -> Battle -> Result` 正式主通路
...
当前可观察到的结果包括：
- 可从正式 `MainMenu` 进入游戏而不依赖浮动预览窗口
- 可在 `HeroSelect` 中配置双方阵容后进入战斗
- 双方单位生成
- 自动前进
- 自动索敌
- 普攻交战
...
- HUD 刷新
- 战斗结束后自动进入 `Result`
- `Result` 页面显示胜负、比分与英雄统计
- 火法当前已经能显示项目专用的普攻投射物、小技能范围爆发和大招持续范围特效
...
- `Battle` 场景当前已切为正式主流程用入口；开发验证入口主要保留在 `BattleBasicAttackOnly` 与编辑器 Dev 菜单。
- 当前主流程已经可用，但 `MainMenu / HeroSelect / Result` 仍是功能优先的第一版界面，不应误判为最终 UI 完成态。
- 当前 prefab-first 战斗 VFX 的完整落地样例主要集中在火法；后续别的英雄应优先复用同样流程，而不是再退回资源包原 prefab 直挂或堆英雄专属表现代码。
...
4. 提升 `HeroSelect / Result` 的可读性和交互质量
5. 补更可读的正式日志面板

原因：
- 现在基础单位循环、技能第一版、职业 AI 第一版和正式主通路都已经有了
- 下一阶段最值得做的是把当前“能跑”的技能系统继续变成“更符合职业定位”的技能系统
- 在主通路已经成立后，UI 可以继续改进，但仍不应压过核心战斗行为优化
