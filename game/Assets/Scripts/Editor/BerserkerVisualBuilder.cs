...
{
    public static class BerserkerVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Berserker Visual";
        private const string BaseSourceSheetPath = "Assets/Art/Heroes/warrior_003_berserker/berserker_source_sheet.png";
        private const string RunSourceSheetPath = BaseSourceSheetPath;
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/warrior_003_berserker";
        private const string ResourcesPrefix = "HeroPreview/warrior_003_berserker";
        private const string BerserkerPrefabPath = "Assets/Prefabs/Heroes/warrior_003_berserker/Berserker.prefab";
        private const string BerserkerPortraitPath = "Assets/Prefabs/Heroes/warrior_003_berserker/Berserker_idle_front.png";
        private const string BerserkerHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/warrior_003_berserker/Berserker.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/BerserkerVisualBuilder.cs";
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("RageIdle", BaseSourceSheetPath, 3, "rage_idle", 7f, true),
            new ClipBuildSpec("Ult", BaseSourceSheetPath, 4, "titan_rage", 12f, false),
            new ClipBuildSpec("RageAttack1", BaseSourceSheetPath, 5, "rage_attack", 16f, false),
            new ClipBuildSpec("Hit", BaseSourceSheetPath, 6, "hit", 12f, false),
            new ClipBuildSpec("Death", BaseSourceSheetPath, 7, "death", 9f, false),
        };
        ...
        public static void BuildBerserkerVisual()
        {
            BuildAll();
            Debug.Log("Berserker sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildBerserkerVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            if (NeedsRebuild())
            {
                BuildBerserkerVisual();
                return;
            }

            SyncBerserkerHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(BerserkerPrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, BaseSourceSheetPath, RunSourceSheetPath)
                > GetLatestTimestampUtc(BerserkerPrefabPath, ResourcesRoot, BerserkerPortraitPath);
        }

        private static void BuildAll()
        {
            GenerateFrameFolders();
            CreateBerserkerPortrait();
            CreateBerserkerPrefab();
            SyncBerserkerHeroAsset();
            ...
            try
            {
                foreach (var spec in ClipBuildSpecs)
                {
                    if (!textureCache.TryGetValue(spec.SourceSheetPath, out var sourceTexture))
                    {
                        ...
                        {
                            throw new FileNotFoundException($"Missing Berserker source sheet: {spec.SourceSheetPath}");
                        }
        ...
        private static void CreateBerserkerPrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/warrior_003_berserker");

            var root = new GameObject("Berserker");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, BerserkerPrefabPath);
            UnityEngine.Object.DestroyImmediate(root);
        }

        private static void CreateBerserkerPortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/warrior_003_berserker");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(BerserkerPortraitPath), true);
            ApplyFrameTextureImporter(BerserkerPortraitPath);
        }
        ...
        private static void SyncBerserkerHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(BerserkerHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(BerserkerPrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(BerserkerPortraitPath);
            if (portrait != null)
            {
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Berserker source row {sourceRow} is outside the detected grid.");
            }
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Berserker source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
