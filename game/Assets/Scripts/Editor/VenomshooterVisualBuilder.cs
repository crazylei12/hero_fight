...
{
    public static class VenomshooterVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Venomshooter Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/marksman_003_venomshooter/venomshooter_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/marksman_003_venomshooter";
        private const string ResourcesPrefix = "HeroPreview/marksman_003_venomshooter";
        private const string VenomshooterPrefabPath = "Assets/Prefabs/Heroes/marksman_003_venomshooter/Venomshooter.prefab";
        private const string VenomshooterPortraitPath = "Assets/Prefabs/Heroes/marksman_003_venomshooter/Venomshooter_idle_front.png";
        private const string VenomshooterHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/marksman_003_venomshooter/Venomshooter.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/VenomshooterVisualBuilder.cs";
        private const int SourceFrameColumns = 8;
        private const int OutputFrameColumns = 9;
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "poison_mist", 14f, false),
            new ClipBuildSpec("Ult", 4, "venom_detonation", 12f, false),
        ...
        public static void BuildVenomshooterVisual()
        {
            BuildAll();
            Debug.Log("Venomshooter sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildVenomshooterVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            if (NeedsRebuild())
            {
                BuildVenomshooterVisual();
                return;
            }

            SyncVenomshooterHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(VenomshooterPrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(VenomshooterPrefabPath, ResourcesRoot, VenomshooterPortraitPath);
        }

        private static void BuildAll()
        {
            GenerateFrameFolders();
            CreateVenomshooterPortrait();
            CreateVenomshooterPrefab();
            SyncVenomshooterHeroAsset();
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing Venomshooter source sheet: {SourceSheetPath}");
            }
            ...
            try
            {
                ...
                foreach (var spec in ClipBuildSpecs)
                {
                    ...
                    var sourceFrameRects = BuildSourceFrameRects(grid, spec.SourceRow);
                    var outputFrameWidth = 1;
                    var outputFrameHeight = 1;
                    foreach (var rect in sourceFrameRects)
                    {
                        outputFrameWidth = Mathf.Max(outputFrameWidth, rect.width);
                        outputFrameHeight = Mathf.Max(outputFrameHeight, rect.height);
                    }

                    for (var i = 0; i < OutputFrameColumns; i++)
                    {
                        var sourceIndex = Mathf.Min(i, SourceFrameColumns - 1);
                        var frameTexture = CropFrame(sourceTexture, sourceFrameRects[sourceIndex], outputFrameWidth, outputFrameHeight);
        ...
        private static void CreateVenomshooterPrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/marksman_003_venomshooter");

            var root = new GameObject("Venomshooter");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, VenomshooterPrefabPath);
            UnityEngine.Object.DestroyImmediate(root);
        }

        private static void CreateVenomshooterPortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/marksman_003_venomshooter");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(VenomshooterPortraitPath), true);
            ApplyFrameTextureImporter(VenomshooterPortraitPath);
        }
        ...
        private static void SyncVenomshooterHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(VenomshooterHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(VenomshooterPrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(VenomshooterPortraitPath);
            if (portrait != null)
            {
        ...
        private static RectInt[] BuildSourceFrameRects(GridSpec grid, int sourceRow)
        {
            if (sourceRow < 0 || sourceRow >= grid.HorizontalLines.Length - 1)
            {
                throw new InvalidOperationException($"Venomshooter source row {sourceRow} is outside the detected grid.");
            }
            ...
            int threshold)
        {
            ...
            {
                groups = SelectEvenlySpacedLineGroups(groups, expectedCount, majorLength);
            }

            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Venomshooter source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
        ...
        private static List<GridLine> SelectEvenlySpacedLineGroups(List<GridLine> groups, int expectedCount, int majorLength)
        {
            var selected = new List<GridLine>(expectedCount);
            var used = new bool[groups.Count];
            var denominator = Mathf.Max(1, expectedCount - 1);

            for (var i = 0; i < expectedCount; i++)
            {
                var expectedPosition = i * (majorLength - 1f) / denominator;
                var bestIndex = -1;
                ...
                for (var j = 0; j < groups.Count; j++)
                {
                    ...
                    var group = groups[j];
                    var center = (group.Min + group.Max) * 0.5f;
                    var distance = Mathf.Abs(center - expectedPosition);
                    if (distance > bestDistance || (Mathf.Approximately(distance, bestDistance) && group.Peak <= bestPeak))
                    {
                        continue;
                    }
                ...
                used[bestIndex] = true;
                selected.Add(groups[bestIndex]);
            }

            selected.Sort((left, right) => left.Min.CompareTo(right.Min));
            return selected;
        }
        ...
        private static bool IsGridLinePixel(Color32 color)
        {
            ...
            var isDarkLine = average <= 95f && max - min <= 35;
            var isMediumLine = average >= 145f && average <= 220f && max - min <= 10;
