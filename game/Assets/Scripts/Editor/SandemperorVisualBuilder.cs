...
{
    public static class SandemperorVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Sandemperor Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/mage_003_sandemperor/sandemperor_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/mage_003_sandemperor";
        private const string ResourcesPrefix = "HeroPreview/mage_003_sandemperor";
        private const string SandemperorPrefabPath = "Assets/Prefabs/Heroes/mage_003_sandemperor/Sandemperor.prefab";
        private const string SandemperorPortraitPath = "Assets/Prefabs/Heroes/mage_003_sandemperor/Sandemperor_idle_front.png";
        private const string SandemperorHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/mage_003_sandemperor/Sandemperor.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/SandemperorVisualBuilder.cs";
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "raise_sandguard", 12f, false),
            new ClipBuildSpec("Ult", 4, "imperial_encirclement", 10f, false),
        ...
        public static void BuildSandemperorVisual()
        {
            BuildAll();
            Debug.Log("Sandemperor sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildSandemperorVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            if (NeedsRebuild())
            {
                BuildSandemperorVisual();
                return;
            }

            SyncSandemperorHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(SandemperorPrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(SandemperorPrefabPath, ResourcesRoot, SandemperorPortraitPath);
        }
        ...
        private static void BuildAll()
        {
            ...
            CreateSandemperorPortrait();
            CreateSandemperorPrefab();
            SyncSandemperorHeroAsset();
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing Sandemperor source sheet: {SourceSheetPath}");
            }
        ...
        private static void CreateSandemperorPortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/mage_003_sandemperor");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(SandemperorPortraitPath), true);
            ApplyFrameTextureImporter(SandemperorPortraitPath);
        }

        private static void CreateSandemperorPrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/mage_003_sandemperor");

            var root = new GameObject("Sandemperor");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, SandemperorPrefabPath);
        ...
        private static void SyncSandemperorHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(SandemperorHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(SandemperorPrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(SandemperorPortraitPath);
            if (portrait != null)
            {
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Sandemperor source row {sourceRow} is outside the detected grid.");
            }
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Sandemperor source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
