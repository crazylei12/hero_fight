...
{
    public static class TidehunterVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Tidehunter Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/tank_003_tidehunter/tidehunter_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/tank_003_tidehunter";
        private const string ResourcesPrefix = "HeroPreview/tank_003_tidehunter";
        private const string TidehunterPrefabPath = "Assets/Prefabs/Heroes/tank_003_tidehunter/Tidehunter.prefab";
        private const string TidehunterPortraitPath = "Assets/Prefabs/Heroes/tank_003_tidehunter/Tidehunter_idle_front.png";
        private const string TidehunterHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/tank_003_tidehunter/Tidehunter.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/TidehunterVisualBuilder.cs";
        private const int SourceFrameColumns = 9;
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "undertow_carapace", 14f, false),
            new ClipBuildSpec("Ult", new[] { 4, 5 }, "tidal_rebound", 12f, false),
        ...
        public static void BuildTidehunterVisual()
        {
            BuildAll();
            Debug.Log("Tidehunter sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildTidehunterVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            if (NeedsRebuild())
            {
                BuildTidehunterVisual();
                return;
            }

            SyncTidehunterHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(TidehunterPrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(TidehunterPrefabPath, ResourcesRoot, TidehunterPortraitPath);
        }
        ...
        private static void BuildAll()
        {
            ...
            CreateTidehunterPortrait();
            CreateTidehunterPrefab();
            SyncTidehunterHeroAsset();
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing Tidehunter source sheet: {SourceSheetPath}");
            }
            ...
            try
            {
                ...
                foreach (var spec in ClipBuildSpecs)
                {
                    ...
                    var frameRects = new List<RectInt>();
                    foreach (var sourceRow in spec.SourceRows)
                    {
                        frameRects.AddRange(BuildFrameRects(grid, sourceRow));
                    }
        ...
        private static void CreateTidehunterPrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/tank_003_tidehunter");

            var root = new GameObject("Tidehunter");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, TidehunterPrefabPath);
            UnityEngine.Object.DestroyImmediate(root);
        }

        private static void CreateTidehunterPortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/tank_003_tidehunter");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(TidehunterPortraitPath), true);
            ApplyFrameTextureImporter(TidehunterPortraitPath);
        }
        ...
        private static void SyncTidehunterHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(TidehunterHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(TidehunterPrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(TidehunterPortraitPath);
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Tidehunter source row {sourceRow} is outside the detected grid.");
            }
        ...
        private static Texture2D CropFrame(Texture2D sourceTexture, RectInt pngRect, int outputFrameWidth, int outputFrameHeight)
        {
            ...
            for (var y = 0; y < pngRect.height && y + verticalPadding < outputFrameHeight; y++)
            {
                ...
                for (var x = 0; x < pngRect.width && x + horizontalPadding < outputFrameWidth; x++)
                {
                    ...
                    var outputIndex = (y + verticalPadding) * outputFrameWidth + x + horizontalPadding;
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Tidehunter source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
        ...
        private static bool IsCheckerBackgroundPixel(Color32 color)
        {
            ...
            var max = Mathf.Max(color.r, Mathf.Max(color.g, color.b));
            var average = (color.r + color.g + color.b) / 3f;
            return average >= 170f && max - min <= 38;
        }
        ...
        private sealed class ClipBuildSpec
        {
            public ClipBuildSpec(
                ...
                bool loop)
                : this(clipKey, new[] { sourceRow }, filePrefix, framesPerSecond, loop)
            {
            }
            ...
            {
                ClipKey = clipKey;
                SourceRows = sourceRows ?? Array.Empty<int>();
