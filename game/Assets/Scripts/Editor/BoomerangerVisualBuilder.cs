...
namespace Fight.Editor
{
    public static class BoomerangerVisualBuilder
    {
        ...
        private const string BoomerangerPortraitPath = "Assets/Prefabs/Heroes/marksman_004_boomeranger/Boomeranger_idle_front.png";
        private const string BoomerangerHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/marksman_004_boomeranger/Boomeranger.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/BoomerangerVisualBuilder.cs";
        ...
        public static void BuildBoomerangerVisual()
        {
            BuildAll();
            Debug.Log("Boomeranger sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildBoomerangerVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            if (NeedsRebuild())
            {
                BuildBoomerangerVisual();
                return;
            }

            SyncBoomerangerHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(BoomerangerPrefabPath) == null)
            {
                return true;
            }
        ...
        private static void BuildAll()
        {
            ...
            CreateBoomerangerPortrait();
            CreateBoomerangerPrefab();
            SyncBoomerangerHeroAsset();
        ...
        {
            EnsureFolder("Assets/Prefabs/Heroes/marksman_004_boomeranger");

            var root = new GameObject("Boomeranger");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, BoomerangerPrefabPath);
            UnityEngine.Object.DestroyImmediate(root);
        }

        private static void CreateBoomerangerPortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/marksman_004_boomeranger");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(BoomerangerPortraitPath), true);
            ApplyFrameTextureImporter(BoomerangerPortraitPath);
        }
        ...
        private static void SyncBoomerangerHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(BoomerangerHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(BoomerangerPrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(BoomerangerPortraitPath);
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Boomeranger source row {sourceRow} is outside the detected grid.");
            }
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Boomeranger source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
        ...
        private static void RemoveInteriorBrightCheckerComponents(Color32[] pixels, int width, int height)
        {
            ...
            for (var y = 0; y < height; y++)
            {
                ...
                {
                    var index = y * width + x;
                    if (labels[index] >= 0 || !IsBrightCheckerPixel(pixels[index]))
                    {
                        continue;
                    }

                    var component = new FrameComponent(label);
                    labels[index] = label;
                    queue.Enqueue(index);
                    ...
                    while (queue.Count > 0)
                    {
                        ...
                        for (var offsetY = -1; offsetY <= 1; offsetY++)
                        {
                            for (var offsetX = -1; offsetX <= 1; offsetX++)
                            {
                                ...
                                if (labels[neighborIndex] >= 0 || !IsBrightCheckerPixel(pixels[neighborIndex]))
                                {
                                    continue;
                                }
        ...
        private static bool IsBrightCheckerPixel(Color32 color)
        {
            ...
            var max = Mathf.Max(color.r, Mathf.Max(color.g, color.b));
            var average = (color.r + color.g + color.b) / 3f;
            return min >= 235 && average >= 238f && max - min <= 12;
        }
