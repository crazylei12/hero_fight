...
{
    public static class FrostmageVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Frostmage Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/mage_002_frostmage/frostmage_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/mage_002_frostmage";
        private const string ResourcesPrefix = "HeroPreview/mage_002_frostmage";
        private const string FrostmagePrefabPath = "Assets/Prefabs/Heroes/mage_002_frostmage/Frostmage.prefab";
        private const string FrostmagePortraitPath = "Assets/Prefabs/Heroes/mage_002_frostmage/Frostmage_idle_front.png";
        private const string FrostmageHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/mage_002_frostmage/Frostmage.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/FrostmageVisualBuilder.cs";
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "frost_burst", 14f, false),
            new ClipBuildSpec("Ult", 4, "blizzard", 12f, false),
        ...
        public static void BuildFrostmageVisual()
        {
            BuildAll();
            Debug.Log("Frostmage sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildFrostmageVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            if (NeedsRebuild())
            {
                BuildFrostmageVisual();
                return;
            }

            SyncFrostmageHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(FrostmagePrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(FrostmagePrefabPath, ResourcesRoot, FrostmagePortraitPath);
        }

        private static void BuildAll()
        {
            GenerateFrameFolders();
            CreateFrostmagePortrait();
            CreateFrostmagePrefab();
            SyncFrostmageHeroAsset();
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing Frostmage source sheet: {SourceSheetPath}");
            }
        ...
        private static void CreateFrostmagePrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/mage_002_frostmage");

            var root = new GameObject("Frostmage");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, FrostmagePrefabPath);
            UnityEngine.Object.DestroyImmediate(root);
        }

        private static void CreateFrostmagePortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/mage_002_frostmage");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(FrostmagePortraitPath), true);
            ApplyFrameTextureImporter(FrostmagePortraitPath);
        }
        ...
        private static void SyncFrostmageHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(FrostmageHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(FrostmagePrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(FrostmagePortraitPath);
            if (portrait != null)
            {
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Frostmage source row {sourceRow} is outside the detected grid.");
            }
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Frostmage source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
