...
{
    public static class SpellbladeVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Spellblade Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/warrior_004_spellblade/spellblade_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/warrior_004_spellblade";
        private const string ResourcesPrefix = "HeroPreview/warrior_004_spellblade";
        private const string SpellbladePrefabPath = "Assets/Prefabs/Heroes/warrior_004_spellblade/Spellblade.prefab";
        private const string SpellbladePortraitPath = "Assets/Prefabs/Heroes/warrior_004_spellblade/Spellblade_idle_front.png";
        private const string SpellbladeHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/warrior_004_spellblade/Spellblade.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/SpellbladeVisualBuilder.cs";
        private const int ExpectedFrameColumns = 8;
        private const int ExpectedActionRows = 7;
        private const int GridCellInsetPixels = 4;
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "rift_wave", 14f, false),
            new ClipBuildSpec("Ult", 4, "bound_blade", 12f, false),
        ...
        public static void BuildSpellbladeVisual()
        {
            BuildAll();
            Debug.Log("Spellblade sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildSpellbladeVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            if (NeedsRebuild())
            {
                BuildSpellbladeVisual();
                return;
            }

            SyncSpellbladeHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(SpellbladePrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(SpellbladePrefabPath, ResourcesRoot, SpellbladePortraitPath);
        }

        private static void BuildAll()
        {
            GenerateFrameFolders();
            CreateSpellbladePortrait();
            CreateSpellbladePrefab();
            SyncSpellbladeHeroAsset();
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing Spellblade source sheet: {SourceSheetPath}");
            }
        ...
        private static void CreateSpellbladePrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/warrior_004_spellblade");

            var root = new GameObject("Spellblade");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, SpellbladePrefabPath);
            UnityEngine.Object.DestroyImmediate(root);
        }

        private static void CreateSpellbladePortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/warrior_004_spellblade");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(SpellbladePortraitPath), true);
            ApplyFrameTextureImporter(SpellbladePortraitPath);
        }
        ...
        private static void SyncSpellbladeHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(SpellbladeHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(SpellbladePrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(SpellbladePortraitPath);
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Spellblade source row {sourceRow} is outside the detected grid.");
            }

            var rects = new RectInt[ExpectedFrameColumns];
            var yMin = grid.HorizontalLines[sourceRow].Max + 1 + GridCellInsetPixels;
            var yMax = grid.HorizontalLines[sourceRow + 1].Min - 1 - GridCellInsetPixels;
            for (var i = 0; i < rects.Length; i++)
            {
                var xMin = grid.VerticalLines[i].Max + 1 + GridCellInsetPixels;
                var xMax = grid.VerticalLines[i + 1].Min - 1 - GridCellInsetPixels;
                rects[i] = new RectInt(
                    xMin,
                    yMin,
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Spellblade source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
            ...
            Queue<int> queue)
        {
            ...
            var index = y * width + x;
            var nearFrameEdge = x <= 2 || y <= 2 || x >= width - 3 || y >= height - 3;
            if (visited[index]
                || (!IsCheckerBackgroundPixel(pixels[index]) && !(nearFrameEdge && IsGridLinePixel(pixels[index]))))
            {
                return;
            }
        ...
        private static void ClearGeneratedPngs(string folderPath)
        {
            ...
            {
                File.Delete(path);
                // Keep .meta files stable when the same frame set is rebuilt.
            }
        }
