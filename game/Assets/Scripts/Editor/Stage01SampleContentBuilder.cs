...
namespace Fight.Editor
{
    public static class Stage01SampleContentBuilder
    {
        ...
        private const string OpenBattleMenuPath = "Fight/Stage 01/Open Battle Scene";
        ...
        private const string FireMageProjectilePrefabPath = "Assets/Super Pixel Projectiles Pack 3/Prefabs/pj3_magic_orb_small_orange.prefab";
        private const string MageUltimateAreaVfxPrefabPath = "Assets/Super Pixel Effects Pack 2/Prefabs/fx2_fire_burst_large_orange.prefab";
        ...
        private static void GenerateDemoContentInternal(bool overwriteExistingContent)
        {
            ...
            var mage = CreateHero(
                ...
                ConfigureMageUltimate(CreateSkill("skill_mage_ultimate_meteor", "Meteor Fall", SkillSlotType.Ultimate, SkillType.AreaDamage, SkillTargetType.DensestEnemyArea, 7f, 3.5f, 2.4f, 16f, 3, overwriteExistingContent), overwriteExistingContent),
                overwriteExistingContent,
            ...
            CreatePlaceholderScenes(overwriteExistingContent);
            ...
            if (!Application.isBatchMode)
            {
                var message = overwriteExistingContent
                    ? "Demo content regenerated and existing tuning was overwritten.\nOpen Assets/Scenes/Battle.unity for the full demo or Assets/Scenes/BattleBasicAttackOnly.unity for basic-attack validation."
                    : "Demo content ensured without overwriting existing tuning.\nOpen Assets/Scenes/Battle.unity for the full demo or use Fight/Stage 01/Open Battle Scene.";
        ...
        private static SkillData CreateMageActiveBurstSkill(bool overwriteExistingContent)
        {
            ...
            skill.skillAreaPresentationType = SkillAreaPresentationType.None;
            AddPersistentAreaDamageEffect(skill, 1.2f, 2f, 0.1f, 1f, false);
            ResetUltimateDecision(skill);
            EditorUtility.SetDirty(skill);
            return skill;
        }
        ...
        private static SkillData ConfigureMageUltimate(SkillData skill, bool overwriteExistingContent)
        {
            ...
            skill.areaRadius = 6f;
            skill.persistentAreaVfxPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(MageUltimateAreaVfxPrefabPath);
            skill.persistentAreaVfxScaleMultiplier = 1.15f;
            skill.skillAreaPresentationType = SkillAreaPresentationType.FireSea;
            skill.effects.Clear();
        ...
        private static void CreatePlaceholderScenes(bool overwriteExistingContent)
        {
            CreatePlaceholderScene(MainMenuScenePath, "MainMenu Placeholder", overwriteExistingContent);
            CreatePlaceholderScene(HeroSelectScenePath, "HeroSelect Placeholder", overwriteExistingContent);
        }

        private static void CreatePlaceholderScene(string scenePath, string label, bool overwriteExistingContent)
        {
        ...
        {
            CreateBattleSceneWithInput(BattleScenePath, battleInput, overwriteExistingContent);
            EditorSceneManager.OpenScene(BattleScenePath, OpenSceneMode.Single);
        ...
        {
            CreateBattleSceneWithInput(BasicAttackOnlyBattleScenePath, battleInput, overwriteExistingContent);
        }

        private static void CreateBattleSceneWithInput(string scenePath, BattleInputConfig battleInput, bool overwriteExistingContent)
        {
            if (!overwriteExistingContent && File.Exists(scenePath))
            {
                return;
            ...
            var bootstrap = root.AddComponent<BattleDebugSceneBootstrap>();

            SetPrivateObjectReference(bootstrap, "defaultInputConfig", battleInput);
            SetPrivateBool(bootstrap, "startBattleOnPlay", true);
            SetPrivateBool(bootstrap, "addBattleHud", true);
            SetPrivateBool(bootstrap, "addBattleView", true);
            SetPrivateBool(bootstrap, "addDebugHud", true);
            SetPrivateBool(bootstrap, "addDebugLogForwarder", true);
