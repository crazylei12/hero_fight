...
{
    public static class ShieldwardenVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Shieldwarden Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/tank_002_shieldwarden/shieldwarden_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/tank_002_shieldwarden";
        private const string ResourcesPrefix = "HeroPreview/tank_002_shieldwarden";
        private const string ShieldwardenPrefabPath = "Assets/Prefabs/Heroes/tank_002_shieldwarden/Shieldwarden.prefab";
        private const string ShieldwardenPortraitPath = "Assets/Prefabs/Heroes/tank_002_shieldwarden/Shieldwarden_idle_front.png";
        private const string ShieldwardenHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/tank_002_shieldwarden/Shieldwarden.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/ShieldwardenVisualBuilder.cs";
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "wardens_call", 14f, false),
            new ClipBuildSpec("Ult", 5, "last_bastion", 12f, false),
        ...
        public static void BuildShieldwardenVisual()
        {
            BuildAll();
            Debug.Log("Shieldwarden sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildShieldwardenVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            if (NeedsRebuild())
            {
                BuildShieldwardenVisual();
                return;
            }

            SyncShieldwardenHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(ShieldwardenPrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(ShieldwardenPrefabPath, ResourcesRoot, ShieldwardenPortraitPath);
        }
        ...
        private static void BuildAll()
        {
            ...
            CreateShieldwardenPortrait();
            CreateShieldwardenPrefab();
            SyncShieldwardenHeroAsset();
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing Shieldwarden source sheet: {SourceSheetPath}");
            }
            ...
            try
            {
                ...
                foreach (var spec in ClipBuildSpecs)
                {
                    ...
                    for (var i = 0; i < OutputFrameCount; i++)
                    {
                        ...
                        var frameTexture = CropFrame(sourceTexture, frameRects[sourceFrame], outputFrameWidth, outputFrameHeight);
        ...
        private static void CreateShieldwardenPrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/tank_002_shieldwarden");

            var root = new GameObject("Shieldwarden");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, ShieldwardenPrefabPath);
            UnityEngine.Object.DestroyImmediate(root);
        }

        private static void CreateShieldwardenPortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/tank_002_shieldwarden");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(ShieldwardenPortraitPath), true);
            ApplyFrameTextureImporter(ShieldwardenPortraitPath);
        }
        ...
        private static void SyncShieldwardenHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(ShieldwardenHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(ShieldwardenPrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(ShieldwardenPortraitPath);
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Shieldwarden source row {sourceRow} is outside the detected grid.");
            }
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Shieldwarden source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
        ...
        private sealed class ClipBuildSpec
        {
            ...
            {
                return Mathf.Clamp(frameIndex, 0, SourceFrameColumns - 1);
            }
        }
    }
}
