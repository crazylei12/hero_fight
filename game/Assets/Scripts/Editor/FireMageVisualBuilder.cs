...
{
    public static class FireMageVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build FireMage Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/mage_001_firemage/firemage_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/mage_001_firemage";
        private const string ResourcesPrefix = "HeroPreview/mage_001_firemage";
        private const string FireMagePrefabPath = "Assets/Prefabs/Heroes/mage_001_firemage/FireMageSprite.prefab";
        private const string FireMagePortraitPath = "Assets/Prefabs/Heroes/mage_001_firemage/FireMageSprite_idle_front.png";
        private const string FireMageHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/mage_001_firemage/FIREMAGE.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/FireMageVisualBuilder.cs";
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "ember_burst", 12f, false),
            new ClipBuildSpec("Ult", 4, "meteor_fall", 10f, false),
        ...
        public static void BuildFireMageVisual()
        {
            BuildAll();
            Debug.Log("FireMage sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildFireMageVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            SyncFireMageHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(FireMagePrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(FireMagePrefabPath, ResourcesRoot, FireMagePortraitPath);
        }
        ...
        private static void BuildAll()
        {
            ...
            CreateFireMagePortrait();
            CreateFireMagePrefab();
            SyncFireMageHeroAsset();
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing FireMage source sheet: {SourceSheetPath}");
            }
        ...
        private static void CreateFireMagePortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/mage_001_firemage");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(FireMagePortraitPath), true);
            ApplyFrameTextureImporter(FireMagePortraitPath);
        }

        private static void CreateFireMagePrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/mage_001_firemage");

            var root = new GameObject("FireMageSprite");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, FireMagePrefabPath);
        ...
        private static void SyncFireMageHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(FireMageHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(FireMagePrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(FireMagePortraitPath);
            if (portrait != null)
            {
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"FireMage source row {sourceRow} is outside the detected grid.");
            }
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"FireMage source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
