...
{
    public static class SkybreakerVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Skybreaker Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/warrior_001_skybreaker/skybreaker_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/warrior_001_skybreaker";
        private const string ResourcesPrefix = "HeroPreview/warrior_001_skybreaker";
        private const string SkybreakerPrefabPath = "Assets/Prefabs/Heroes/warrior_001_skybreaker/Skybreaker.prefab";
        private const string SkybreakerPortraitPath = "Assets/Prefabs/Heroes/warrior_001_skybreaker/Skybreaker_idle_front.png";
        private const string SkybreakerHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/warrior_001_skybreaker/Skybreaker.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/SkybreakerVisualBuilder.cs";
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "breaker_rush", 14f, false),
            new ClipBuildSpec("Ult", 4, "skyquake", 12f, false),
        ...
        public static void BuildSkybreakerVisual()
        {
            BuildAll();
            Debug.Log("Skybreaker sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildSkybreakerVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            if (NeedsRebuild())
            {
                BuildSkybreakerVisual();
                return;
            }

            SyncSkybreakerHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(SkybreakerPrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(SkybreakerPrefabPath, ResourcesRoot, SkybreakerPortraitPath);
        }

        private static void BuildAll()
        {
            GenerateFrameFolders();
            CreateSkybreakerPortrait();
            CreateSkybreakerPrefab();
            SyncSkybreakerHeroAsset();
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing Skybreaker source sheet: {SourceSheetPath}");
            }
        ...
        private static void CreateSkybreakerPrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/warrior_001_skybreaker");

            var root = new GameObject("Skybreaker");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, SkybreakerPrefabPath);
            UnityEngine.Object.DestroyImmediate(root);
        }

        private static void CreateSkybreakerPortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/warrior_001_skybreaker");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(SkybreakerPortraitPath), true);
            ApplyFrameTextureImporter(SkybreakerPortraitPath);
        }
        ...
        private static void SyncSkybreakerHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(SkybreakerHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(SkybreakerPrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(SkybreakerPortraitPath);
            if (portrait != null)
            {
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Skybreaker source row {sourceRow} is outside the detected grid.");
            }
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Skybreaker source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
