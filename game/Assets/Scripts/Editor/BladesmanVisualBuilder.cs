...
namespace Fight.Editor
{
    public static class BladesmanVisualBuilder
    {
        ...
        private const string BladesmanPortraitPath = "Assets/Prefabs/Heroes/warrior_002_bladesman/Bladesman_idle_front.png";
        private const string BladesmanHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/warrior_002_bladesman/Bladesman.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/BladesmanVisualBuilder.cs";
        ...
        public static void BuildBladesmanVisual()
        {
            BuildAll();
            Debug.Log("Bladesman sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildBladesmanVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            if (NeedsRebuild())
            {
                BuildBladesmanVisual();
                return;
            }

            SyncBladesmanHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(BladesmanPrefabPath) == null)
            {
                return true;
            }
        ...
        private static void BuildAll()
        {
            ...
            CreateBladesmanPortrait();
            CreateBladesmanPrefab();
            SyncBladesmanHeroAsset();
        ...
        {
            EnsureFolder("Assets/Prefabs/Heroes/warrior_002_bladesman");

            var root = new GameObject("Bladesman");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, BladesmanPrefabPath);
            UnityEngine.Object.DestroyImmediate(root);
        }

        private static void CreateBladesmanPortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/warrior_002_bladesman");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(BladesmanPortraitPath), true);
            ApplyFrameTextureImporter(BladesmanPortraitPath);
        }
        ...
        private static void SyncBladesmanHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(BladesmanHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(BladesmanPrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(BladesmanPortraitPath);
            if (portrait != null)
            {
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Bladesman source row {sourceRow} is outside the detected grid.");
            }
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Bladesman source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
