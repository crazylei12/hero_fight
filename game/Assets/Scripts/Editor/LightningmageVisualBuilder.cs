...
{
    public static class LightningmageVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Lightningmage Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/mage_004_lightningmage/lightningmage_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/mage_004_lightningmage";
        private const string ResourcesPrefix = "HeroPreview/mage_004_lightningmage";
        private const string LightningmagePrefabPath = "Assets/Prefabs/Heroes/mage_004_lightningmage/Lightningmage.prefab";
        private const string LightningmagePortraitPath = "Assets/Prefabs/Heroes/mage_004_lightningmage/Lightningmage_idle_front.png";
        private const string LightningmageHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/mage_004_lightningmage/Lightningmage.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/LightningmageVisualBuilder.cs";
        private const int ExpectedFrameColumns = 9;
        private const int ExpectedActionRows = 7;
        private const float GridLineCoverageRatio = 0.72f;
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "thunderline", 14f, false),
            new ClipBuildSpec("Ult", 4, "storm_verdict", 12f, false),
        ...
        public static void BuildLightningmageVisual()
        {
            BuildAll();
            Debug.Log("Lightningmage sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildLightningmageVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static void TryAutoBuildIfNeeded()
        {
            ...
            if (NeedsRebuild())
            {
                BuildLightningmageVisual();
                return;
            }

            SyncLightningmageHeroAsset();
            AssetDatabase.SaveAssets();
        }

        private static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(LightningmagePrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(LightningmagePrefabPath, ResourcesRoot, LightningmagePortraitPath);
        }

        private static void BuildAll()
        {
            GenerateFrameFolders();
            CreateLightningmagePortrait();
            CreateLightningmagePrefab();
            SyncLightningmageHeroAsset();
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing Lightningmage source sheet: {SourceSheetPath}");
            }
        ...
        private static void CreateLightningmagePrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/mage_004_lightningmage");

            var root = new GameObject("Lightningmage");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, LightningmagePrefabPath);
            UnityEngine.Object.DestroyImmediate(root);
        }

        private static void CreateLightningmagePortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/mage_004_lightningmage");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(LightningmagePortraitPath), true);
            ApplyFrameTextureImporter(LightningmagePortraitPath);
        }
        ...
        private static void SyncLightningmageHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(LightningmageHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(LightningmagePrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(LightningmagePortraitPath);
            if (portrait != null)
            {
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Lightningmage source row {sourceRow} is outside the detected grid.");
            }
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Lightningmage source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
        ...
        private static bool IsCheckerBackgroundPixel(Color32 color)
        {
            ...
            var max = Mathf.Max(color.r, Mathf.Max(color.g, color.b));
            var average = (color.r + color.g + color.b) / 3f;
            var colorSpread = max - min;
            var isCheckerFill = average >= 218f && colorSpread <= 8;
            var isGridBorderLine = average >= 150f && average <= 235f && colorSpread <= 8;
