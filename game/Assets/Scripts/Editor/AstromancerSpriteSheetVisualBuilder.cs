...
{
    public static class AstromancerSpriteSheetVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Astromancer Sprite Sheet Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/mage_005_astromancer/astromancer_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/mage_005_astromancer";
        private const string ResourcesPrefix = "HeroPreview/mage_005_astromancer";
        private const string HeroAssetPath = "Assets/Data/Stage01Demo/Heroes/mage_005_astromancer/Astromancer.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/AstromancerSpriteSheetVisualBuilder.cs";
        private const int ExpectedFrameColumns = 9;
        private const int ExpectedActionRows = 7;
        private const float GridLineCoverageRatio = 0.62f;
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "falling_star", 12f, false),
            new ClipBuildSpec("Ult", 4, "meteor_choir", 10f, false),
            new ClipBuildSpec("Hit", 5, "hit", 10f, false),
            new ClipBuildSpec("Death", 6, "death", 8f, false),
        };

        [MenuItem(BuildMenuPath)]
        public static void BuildAstromancerSpriteSheetVisual()
        {
            BuildHeroVisual();
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            Debug.Log("Astromancer sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildAstromancerSpriteSheetVisualBatch()
        {
            try
            {
                BuildAstromancerSpriteSheetVisual();
        ...
        public static void BuildHeroVisual()
        {
            AssetDatabase.ImportAsset(SourceSheetPath, ImportAssetOptions.ForceUpdate);
            GenerateFrameFolders();
            CreateAstromancerPortrait();
            CreateAstromancerPrefab();
            SyncAstromancerHeroAsset();
        }

        public static bool NeedsRebuild()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(AstromancerVisualAndVfxBuilder.HeroPrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(
                    AstromancerVisualAndVfxBuilder.HeroPrefabPath,
                    ResourcesRoot,
                    AstromancerVisualAndVfxBuilder.HeroPortraitPath);
        }
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing Astromancer source sheet: {SourceSheetPath}");
            }
        ...
        private static void CreateAstromancerPortrait()
        {
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(AstromancerVisualAndVfxBuilder.HeroPortraitPath), true);
            ApplyFrameTextureImporter(AstromancerVisualAndVfxBuilder.HeroPortraitPath);
        }

        private static void CreateAstromancerPrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/mage_005_astromancer");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, AstromancerVisualAndVfxBuilder.HeroPrefabPath);
        ...
        private static void SyncAstromancerHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(HeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(AstromancerVisualAndVfxBuilder.HeroPrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(AstromancerVisualAndVfxBuilder.HeroPortraitPath);
            if (portrait != null)
            {
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Astromancer source row {sourceRow} is outside the detected grid.");
            }
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Astromancer source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
        ...
        private static bool IsGridLinePixel(Color32 color)
        {
            ...
            return average <= 116f && max - min <= 65;
        }
            ...
            Queue<int> queue)
        {
            ...
            var index = y * width + x;
            if (visited[index] || !IsSheetBackgroundPixel(pixels[index]))
            {
                return;
            }
        ...
        private static bool IsSheetBackgroundPixel(Color32 color)
        {
            if (color.a == 0)
            {
                return false;
