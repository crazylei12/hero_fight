...
{
    public static class LonerVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Loner Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/assassin_004_loner/loner_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/assassin_004_loner";
        private const string ResourcesPrefix = "HeroPreview/assassin_004_loner";
        private const string LonerPrefabPath = "Assets/Prefabs/Heroes/assassin_004_loner/Loner.prefab";
        private const string LonerPortraitPath = "Assets/Prefabs/Heroes/assassin_004_loner/Loner_idle_front.png";
        private const string LonerHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/assassin_004_loner/Loner.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/LonerVisualBuilder.cs";
        private const int ExpectedFrameColumns = 9;
        private const int ExpectedActionRows = 7;
        private const float GridLineCoverageRatio = 0.80f;
        ...
        private const int SkillOverflowSourceRow = 3;
        private const int SkillOverflowFrameIndex = 4;
        private const int SkillOverflowRightPixels = 28;
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "lone_pursuit", 12f, false),
            new ClipBuildSpec("Ult", 4, "lone_instinct", 10f, false),
        ...
        public static void BuildLonerVisual()
        {
            BuildAll();
            Debug.Log("Loner sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildLonerVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(LonerPrefabPath) == null)
            {
                return true;
            }
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(LonerPrefabPath, ResourcesRoot, LonerPortraitPath);
        }
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing Loner source sheet: {SourceSheetPath}");
            }
        ...
        private static void CreateLonerPortrait()
        {
            EnsureFolder("Assets/Prefabs/Heroes/assassin_004_loner");
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(LonerPortraitPath), true);
            ApplyFrameTextureImporter(LonerPortraitPath);
        }

        private static void CreateLonerPrefab()
        {
            EnsureFolder("Assets/Prefabs/Heroes/assassin_004_loner");

            var root = new GameObject("Loner");
            ...
            PrefabUtility.SaveAsPrefabAsset(root, LonerPrefabPath);
        ...
        private static void SyncLonerHeroAsset()
        {
            var hero = AssetDatabase.LoadAssetAtPath<HeroDefinition>(LonerHeroAssetPath);
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(LonerPrefabPath);
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(LonerPortraitPath);
            if (portrait != null)
            {
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Loner source row {sourceRow} is outside the detected grid.");
            }
            ...
            for (var i = 0; i < rects.Length; i++)
            {
                ...
                if (sourceRow == SkillOverflowSourceRow && i == SkillOverflowFrameIndex)
                {
                    xMax = Mathf.Min(
                        grid.VerticalLines[i + 2].Min - 1 - GridLinePaddingPixels,
                        xMax + SkillOverflowRightPixels);
                }
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Loner source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
        ...
        private static bool IsCheckerBackgroundPixel(Color32 color)
        {
            ...
            var max = Mathf.Max(color.r, Mathf.Max(color.g, color.b));
            var average = (color.r + color.g + color.b) / 3f;
            var isCheckerFill = average >= 190f && max - min <= 30;
