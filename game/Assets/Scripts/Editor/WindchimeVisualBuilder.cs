...
{
    public static class WindchimeVisualBuilder
    {
        private const string BuildMenuPath = "Fight/Stage 01/Build Windchime Visual";
        private const string SourceSheetPath = "Assets/Art/Heroes/support_002_windchime/windchime_source_sheet.png";
        private const string ResourcesRoot = "Assets/Resources/HeroPreview/support_002_windchime";
        private const string ResourcesPrefix = "HeroPreview/support_002_windchime";
        private const string WindchimePrefabPath = "Assets/Prefabs/Heroes/support_002_windchime/Windchime.prefab";
        private const string WindchimePortraitPath = "Assets/Prefabs/Heroes/support_002_windchime/Windchime_idle_front.png";
        private const string WindchimeHeroAssetPath = "Assets/Data/Stage01Demo/Heroes/support_002_windchime/Windchime.asset";
        private const string BuilderScriptAssetPath = "Assets/Scripts/Editor/WindchimeVisualBuilder.cs";
        ...
        private static readonly ClipBuildSpec[] ClipBuildSpecs =
        {
            ...
            new ClipBuildSpec("Skill", 3, "echo_canopy", 14f, false),
            new ClipBuildSpec("Ult", 4, "stillwind_domain", 12f, false),
        ...
        public static void BuildWindchimeVisual()
        {
            BuildAll();
            Debug.Log("Windchime sprite-sheet visual prefab rebuilt.");
        }

        public static void BuildWindchimeVisualBatch()
        {
            try
            {
                BuildAll();
        ...
        private static bool NeedsRebuild()
        {
            ...
            return GetLatestTimestampUtc(BuilderScriptAssetPath, SourceSheetPath)
                > GetLatestTimestampUtc(WindchimePrefabPath, ResourcesRoot, WindchimePortraitPath);
        }
        ...
        private static void GenerateFrameFolders()
        {
            ...
            {
                throw new FileNotFoundException($"Missing Windchime source sheet: {SourceSheetPath}");
            }
        ...
        private static void CreateWindchimePortrait()
        {
            ...
            File.Copy(absoluteSourcePath, ToAbsolutePath(WindchimePortraitPath), true);
            ApplyFrameTextureImporter(WindchimePortraitPath);
        }
        ...
        private static void CreateWindchimePrefab()
        {
            ...
            var root = new GameObject("Windchime");
        ...
        private static void SyncWindchimeHeroAsset()
        {
            ...
            var portrait = AssetDatabase.LoadAssetAtPath<Sprite>(WindchimePortraitPath);
            if (portrait != null)
            {
        ...
        private static RectInt[] BuildFrameRects(GridSpec grid, int sourceRow)
        {
            ...
            {
                throw new InvalidOperationException($"Windchime source row {sourceRow} is outside the detected grid.");
            }
        ...
        private static Texture2D CropFrame(Texture2D sourceTexture, RectInt pngRect, int outputFrameWidth, int outputFrameHeight)
        {
            ...
            RemoveConnectedBackground(topLeftPixels, outputFrameWidth, outputFrameHeight);
            ...
            int threshold)
        {
            ...
            if (groups.Count != expectedCount)
            {
                throw new InvalidOperationException(
                    $"Windchime source grid detection expected {expectedCount} {(scanColumns ? "vertical" : "horizontal")} lines, found {groups.Count}.");
            }
        ...
        private static void RemoveConnectedBackground(Color32[] pixels, int width, int height)
        {
            ...
            Queue<int> queue)
        {
            ...
            var index = y * width + x;
            if (visited[index] || !IsBackgroundPixel(pixels[index]))
            {
                return;
            }
        ...
        private static bool IsBackgroundPixel(Color32 color)
        {
            if (color.a == 0)
            {
                return false;
            ...
            var max = Mathf.Max(color.r, Mathf.Max(color.g, color.b));
            var average = (color.r + color.g + color.b) / 3f;
            return average >= 214f && max - min <= 48;
        }
